...
class AsterPerpKlineClientManager(KlineClientManager):
    ...
    def get_feed(self, symbol: str) -> Optional['AsterPerpKlineFeed']:
...
class AsterPerpOrderBookClientManager(OrderBookClientManager):
    ...
    def get_feed(self, symbol: str) -> Optional['AsterPerpOrderBookFeed']:
        return self._active_feeds.get(symbol)
...
# Docs: POST /fapi/v1/listenKey  PUT /fapi/v1/listenKey  DELETE /fapi/v1/listenKey
# Stream: wss://fstream.asterdex.com/ws/<listenKey>
# ──────────────────────────────────────────────────────────────────────────────

class AsterPerpUserDataClientManager:
    """
    Manages the Aster futures user-data WebSocket stream.

    Lifecycle:
      1. POST /fapi/v1/listenKey  -> obtain listen key
      2. Connect to wss://fstream.asterdex.com/ws/<listenKey>
      3. PUT /fapi/v1/listenKey every 30 min to keep it alive
      4. Dispatch inbound events to registered callbacks
      5. DELETE /fapi/v1/listenKey on stop
    """
    ...
    def _get_listen_key(self) -> str:
        resp = requests.post(
            f"{FUTURES_BASE_ENDPOINT}/fapi/v1/listenKey",
    ...
    async def _reader_loop(self):
        ...
        while self._running:
            ...
            ws_url = f"{WEBSOCKET_FUTURES_STREAM}/{self._listen_key}"
            try:
