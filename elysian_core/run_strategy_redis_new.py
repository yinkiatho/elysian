...
class StrategyRunner:
    ...
    def __init__(
        ...
        self.logger.info(
            ...
            f'yamls={strategy_config_yamls}, redis={redis_url}'
        )
        ...
        # Main exchange connectors (unauthenticated — for REST market ops)
        self.exchange_connectors_dict = {
            AssetType.SPOT: {Venue.BINANCE: None, Venue.ASTER: None},
            AssetType.PERPETUAL: {Venue.BINANCE: None, Venue.ASTER: None},
        }
    ...
    def _setup_config(self) -> None:
        ...
        self.logger.info(
            f"[Runner] Symbols — Binance: {len(self.binance_token_symbols)}, "
            f"BinanceFut: {len(self.binance_futures_token_symbols)}, "
            f"Aster: {len(self.aster_token_symbols)}"
        )

    # ── Main exchanges (no feed managers — pass None) ─────────────────────────

    def _setup_exchanges(self) -> None:
        if self.binance_token_symbols and 'Binance' in self.cfg.meta.spot_venues:
            self.exchange_connectors_dict[AssetType.SPOT][Venue.BINANCE] = BinanceSpotExchange(
                args=self.cfg,
        ...
        if self.binance_futures_token_symbols and 'Binance' in self.cfg.meta.futures_venues:
            self.exchange_connectors_dict[AssetType.PERPETUAL][Venue.BINANCE] = BinanceFuturesExchange(
                args=self.cfg,
                api_key=self.cfg.secrets.binance.api_key,
                api_secret=self.cfg.secrets.binance.api_secret,
        ...
        if self.aster_token_symbols and 'Aster' in self.cfg.meta.spot_venues:
            self.exchange_connectors_dict[AssetType.SPOT][Venue.ASTER] = AsterSpotExchange(
                args=self.cfg,
    ...
    async def run(self) -> None:
        ...
        for strategy_id, strat_cfg in self.cfg.strategies.items():
            ...
            strategy_names.append(strat_cfg.strategy_name)
            strategy = load_strategy_class(strat_cfg.class_name)(
        ...
        try:
            ...
            try:
                ...
                await asyncio.gather(*[
                    self.setup_strategy(s, sid, sname)
                    for s, sid, sname in zip(strategies, strategy_ids, strategy_names)
                ])
...
async def run_strategy(
    ...
    strategy_config_yamls: List[str],
    redis_url: str,
) -> None:
...
if __name__ == "__main__":
    ...
    asyncio.run(run_strategy(
        ...
        strategy_config_yamls=[args.strategy_yaml],
